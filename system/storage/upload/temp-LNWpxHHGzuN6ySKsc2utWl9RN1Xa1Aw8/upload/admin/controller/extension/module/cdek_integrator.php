...
class ControllerExtensionModuleCdekIntegrator extends Controller {
	...
	public function createOrder() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateOrderFrom()) {
			...
			if ($this->config->get('cdek_log')) {
				$this->log->write('СДЭК: код ответа сервера');
				$this->log->write(print_r($this->api->http_status, 1));
	...
	public function updateCities() {
		...
		foreach ($countries as $country) {
			...
			for ($i = 0; ; $i++) {
				...
				$url = 'https://integration.cdek.ru/v1/location/cities/json?size=10000&countryCode=' . $country . '&page=' . $i;  
				$ch = curl_init();
