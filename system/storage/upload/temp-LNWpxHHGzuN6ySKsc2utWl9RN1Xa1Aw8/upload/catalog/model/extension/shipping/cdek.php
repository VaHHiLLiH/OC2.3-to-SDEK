	...
	function getQuote($address)
	{
		...
		if ($to_data && $address['city']) {
            ...
            foreach ($cities as $city) {
                ...
                $cdek_cities = $this->getURL($this->getInfo()->getBaseUrl().'v1/location/cities/json?cityName=' . urlencode($city), new parser_json());
            ...
            if (is_array($cdek_cities)) {
                ...
                if ($status) {
                    ...
                    foreach ($cdek_cities as $city_info) {
                        ...
                        if (!$empty_country && !in_array($this->_clear($city_info['country']), $countries)) {
                            continue;
                        }

                        list($city)= explode(',', $city_info['cityName']);
...
*/
				if ($count = count($available)) {
                    ...
                    $city_to = $available_city['cityCode'];
